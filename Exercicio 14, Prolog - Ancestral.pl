% Um ancestral é quem dá origem a uma linhagem, chamar uma query que lista decendentes de "ancestral" foi uma pessima escolha, se não fosse o banco de dados eu teria feito errado, pois leva a entender que o resultado deve ser a lista de quem vem ANTES de urano não depois
...
% Exercício 4: Regra ancestral(A, D)
ancestral(A, D) :-
    progenitor(A, D).
